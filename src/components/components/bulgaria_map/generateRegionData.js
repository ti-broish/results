...
export const generateDisplayParties = (parties, results, count, firstPartyId, lastPartyId, removePartyId) => {
    const tempParties = {};
    ...
    if(removePartyId) {
        displayParties = displayParties.filter(party => party.id.toString() !== removePartyId.toString());
    }
    
    if(firstPartyId) {
        firstParty = displayParties.find(party => party.id.toString() === firstPartyId.toString());
        displayParties = displayParties.filter(party => party.id.toString() !== firstPartyId.toString());
        count = count - 1;
    }

    if(lastPartyId) {
        lastParty = displayParties.find(party => party.id.toString() === lastPartyId.toString());
        displayParties = displayParties.filter(party => party.id.toString() !== lastPartyId.toString());
        count = count - 1;
    }
...
export const generateRegionDataDominant = (regions, parties, results) => {
    ...
    for(const region of regions) {
        ...
        const { displayParties, displayPartiesTotal } = generateDisplayParties(parties, region.results, 6, null, '0');
        regionData[region.id].color = displayParties[0].validVotes? `#${displayParties[0].color}` : '#eee';
        regionData[region.id].tooltipData = { displayParties: displayParties, displayPartiesTotal: displayPartiesTotal };
    }

    return regionData;
};

export const generateRegionDataSingleParty = (singleParty, singlePartyMode, regions, parties, results) => {
    const regionData = {};
    ...
    if(!singleParty) {
        ...
        for(const region of regions) {
            regionData[region.id] = {};
            regionResults[region.id] = generateDisplayParties(parties, region.results, 10, null, '0');
        }

        let highestResult = 0;
        let lowestResult = 1000000000;
        for(const region of regions) {
            const regionPartyResult = regionResults[region.id].displayParties.find(party => party.number === singleParty);
...
export const generateRegionDataTurnout = (regions, parties) => {
    ...
    for(const region of regions) {
        const currentTurnout = (region.stats.validVotes + region.stats.invalidVotes) / region.stats.voters;
