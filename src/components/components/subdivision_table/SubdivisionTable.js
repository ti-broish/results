...
import { sortTableDistribution, sortTableVoters, sortTableTurnout } from './sortSubdivisionTable';
...
export default props => {
    ...
    useEffect(() => { ReactTooltip.rebuild(); }, [depthMode]);
    ...
    const sorted = subdivisions => {
        switch(mode) {
            case 'distribution': return sortTableDistribution(subdivisions, singleParty);
            case 'voters': return sortTableVoters(subdivisions);
            case 'turnout': return sortTableTurnout(subdivisions);
        }
    };
    ...
    const getMiddleNodes = () => {
        const nodes = [];
        props.subdivisions.forEach(node => node.nodes.forEach(node => nodes.push(node)));
        return nodes;
    };
    ...
    const { displayParties } = generateDisplayParties(props.parties, props.results, 6, null, null, '0');
    ...
    return (
        <div>
            ...
            <div>
                ...
                {maxDepth > 1? [
                    'Покажи: ',
                    <button className={depthMode === 'showAll'? 'selected' : ''} onClick={()=>setDepthMode('showAll')}>
                        Всичко
                    </button>,
                    <button className={depthMode === 'showTopNodes'? 'selected' : ''} onClick={()=>setDepthMode('showTopNodes')}>
                        {mapNodeType(getTopNodeType())}
                    </button> ] : null}
                {maxDepth >= 2? 
                    <button className={depthMode === 'showMiddleNodes'? 'selected' : ''} onClick={()=>setDepthMode('showMiddleNodes')}>
                ...
                <SubdivisionTableControls embed={props.embed}>
                    ...
                    <button className={mode === 'voters'? 'selected' : ''}  onClick={()=>{
                        //if(maxDepth != 1 && depthMode === 'showAll') setDepthMode('showBottomNodes');
                        setMode('voters');
                    }}>
                        Избиратели
                    </button>
                    <button className={mode === 'turnout'? 'selected' : ''}  onClick={()=>{
                        //if(maxDepth != 1 && depthMode === 'showAll') setDepthMode('showBottomNodes');
                        setMode('turnout');
                    }}>
                        Активност
                ...
                { 
                    mode !== 'distribution'? null : [
                        'Подреди по партия: ',
                        <button className={singleParty === ''? 'selected' : ''} onClick={() => {
                            setSingleParty('');
                        }}>Никоя</button>,
                        displayParties.map(party =>
                            <button style={{fontSize: '12px'}} className={singleParty === party.number? 'selected' : ''} onClick={()=>{
                                //if(maxDepth != 1 && depthMode === 'showAll') setDepthMode('showBottomNodes');
