...
export default props => {
    ...
    const fakeResults = (deviation, voters) => {
        const results = [];
    ...
    const populateWithFakeResults = data => {
        if(data.stats) {
            const deviation = data.type === 'election'? 0.15 : data.type === 'electionRegion'? 0.4 : 0.15;
            const { results, voters, turnout } = fakeResults(deviation, 10000 + 10000 * Math.random());
            data.results = results;
            data.stats.voters = voters;
            data.stats.validVotes = voters * turnout;
        }
        if(data.nodes)
            data.nodes.forEach(node => populateWithFakeResults(node));
        return data;
    };
